...
export function getAppLogoMaskableSvg(primary = '#7c3aed', primaryGlow = '#a78bfa', accent = '#06b6d4', size = 512) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="${size}" height="${size}">
  <defs>
    <!-- Background Gradient -->
    <linearGradient id="lsi-bg" x1="0%" y1="0%" x2="100%" y2="100%">
  ...
  <!-- Full-bleed background for Android Adaptive Icon safe-zone -->
  <rect width="512" height="512" fill="url(#lsi-bg)" />

  <!-- Centered and scaled inside 80% safe zone circle (diameter 409px) -->
  <g transform="translate(256, 256) scale(0.72) translate(-256, -256)">
    <!-- Inner Squircle Container -->
    <rect x="20" y="20" width="472" height="472" rx="120" fill="url(#lsi-bg)" stroke="url(#lsi-border)" stroke-width="12" />
...
async function buildAssets() {
  ...
  for (const { size, out } of standardSizes) {
...
<body style="margin:0;padding:0;background:transparent;display:flex;align-items:center;justify-content:center;width:${size}px;height:${size}px;">
  ...
  // Render maskable and Apple Touch icons
  const maskableSizes = [
    { size: 512, out: path.join(rootDir, 'public', 'icon-maskable-512.png') },
    { size: 192, out: path.join(rootDir, 'public', 'icon-maskable-192.png') },
    { size: 180, out: path.join(rootDir, 'public', 'apple-touch-icon.png') },
    { size: 180, out: path.join(rootDir, 'public', 'apple-touch-icon-180x180.png') },
  ];

  for (const { size, out } of maskableSizes) {
    const html = `<!DOCTYPE html>
<html>
<body style="margin:0;padding:0;background:#0d091a;display:flex;align-items:center;justify-content:center;width:${size}px;height:${size}px;">
  ${getAppLogoMaskableSvg('#7c3aed', '#a78bfa', '#06b6d4', size)}
</body>
</html>`;
    await page.setViewportSize({ width: size, height: size });
    await page.setContent(html);
    await page.screenshot({ path: out, omitBackground: false });
    console.log(`Generated maskable ${path.basename(out)} (${size}x${size})`);
  }
